...
func (r *Repository) CreateCourse(ctx context.Context, customerID, createdBy int, title, slug, summary, descMD string) (*model.Course, error) {
...
func (r *Repository) ListCourses(ctx context.Context, customerID int, status, search string, limit, offset int) ([]model.Course, int, error) {
...
func (r *Repository) GetCourse(ctx context.Context, customerID, id int) (*model.Course, error) {
	const q = `SELECT ` + courseCols + ` FROM courses WHERE customer_id = $1 AND id = $2`
	return scanCourse(r.db.QueryRow(ctx, q, customerID, id))
}

func (r *Repository) UpdateCourse(ctx context.Context, customerID, id int, title, summary, descMD string) (*model.Course, error) {
...
func (r *Repository) SetStatus(ctx context.Context, customerID, id int, status string, publishedAt *time.Time) (*model.Course, error) {
...
func (r *Repository) SetThumbnail(ctx context.Context, customerID, id int, key string) (*model.Course, error) {
...
func (r *Repository) LoadModules(ctx context.Context, courseID int) ([]model.CourseModule, error) {
	...
	mods := make([]model.CourseModule, 0)
	idx := map[int]int{}
	for rows.Next() {
		m := model.CourseModule{Lessons: []model.Lesson{}}
...
func (r *Repository) CreateModule(ctx context.Context, courseID int, title string, position int) (*model.CourseModule, error) {
	m := &model.CourseModule{Lessons: []model.Lesson{}}
	err := r.db.QueryRow(ctx,
...
func (r *Repository) CreateLesson(ctx context.Context, moduleID int, title, contentMD string, position int) (*model.Lesson, error) {
	l := &model.Lesson{}
	err := r.db.QueryRow(ctx,
...
func (r *Repository) CreateResource(ctx context.Context, courseID int, lessonID *int, createdBy int, name, key, contentType string, size int64) (*model.Resource, error) {
	res := &model.Resource{}
	err := r.db.QueryRow(ctx,
...
func (r *Repository) ListResources(ctx context.Context, courseID, limit, offset int) ([]model.Resource, int, error) {
	var total int
...
func (r *Repository) GetResource(ctx context.Context, customerID, courseID, resourceID int) (*model.Resource, error) {
	res := &model.Resource{}
	err := r.db.QueryRow(ctx,
