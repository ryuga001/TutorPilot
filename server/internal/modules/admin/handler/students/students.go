...
func failErr(c *gin.Context, err error) {
	...
	case errors.Is(err, model.ErrNoStudentRole):
...
func (h *Handler) Create(c *gin.Context) {
	...
	st, err := h.svc.Create(c.Request.Context(), customerID, userID, req)
	if err != nil {
		failErr(c, err)
		return
	}
	httpx.OK(c, http.StatusCreated, "student created", st)
}
...
func (h *Handler) Get(c *gin.Context) {
	...
	customerID, _ := h.ids(c)
	st, err := h.svc.Get(c.Request.Context(), customerID, id)
	if err != nil {
		failErr(c, err)
		return
	}
	httpx.OK(c, http.StatusOK, "ok", st)
}
...
func (h *Handler) Update(c *gin.Context) {
	...
	customerID, _ := h.ids(c)
	st, err := h.svc.Update(c.Request.Context(), customerID, id, req)
	if err != nil {
		failErr(c, err)
		return
	}
	httpx.OK(c, http.StatusOK, "student updated", st)
}
...
func (h *Handler) Delete(c *gin.Context) {
	...
	httpx.OK(c, http.StatusOK, "student deleted", nil)
}
...
func (h *Handler) UploadProfileImage(c *gin.Context) {
	...
	customerID, _ := h.ids(c)
	st, err := h.svc.UploadProfileImage(c.Request.Context(), customerID, id, fh)
	if err != nil {
		failErr(c, err)
		return
	}
	httpx.OK(c, http.StatusOK, "profile image updated", st)
}
