...
func (s *Service) Create(ctx context.Context, customerID, userID int, req CreateCourseRequest) (*CourseView, error) {
...
func (s *Service) List(ctx context.Context, customerID int, status, search string, p httpx.Page) (httpx.Paginated[CourseView], error) {
	rows, total, err := s.repo.ListCourses(ctx, customerID, status, search, p.Limit(), p.Offset())
	if err != nil {
		return httpx.Paginated[CourseView]{}, err
	}
...
func (s *Service) Get(ctx context.Context, customerID, id int) (*CourseView, error) {
...
func (s *Service) Update(ctx context.Context, customerID, id int, req UpdateCourseRequest) (*CourseView, error) {
...
func (s *Service) SetPublished(ctx context.Context, customerID, id int, published bool) (*CourseView, error) {
	status := StatusDraft
	var at *time.Time
	if published {
...
func (s *Service) CreateModule(ctx context.Context, customerID, courseID int, req ModuleRequest) (*ModuleView, error) {
...
func (s *Service) UpdateModule(ctx context.Context, customerID, moduleID int, req ModuleRequest) error {
...
func (s *Service) CreateLesson(ctx context.Context, customerID, moduleID int, req LessonRequest) (*LessonView, error) {
	if _, err := s.repo.moduleOwned(ctx, customerID, moduleID); err != nil {
		return nil, err
	}
...
func (s *Service) UpdateLesson(ctx context.Context, customerID, lessonID int, req LessonRequest) error {
...
func (s *Service) ListResources(ctx context.Context, customerID, courseID int, p httpx.Page) (httpx.Paginated[ResourceView], error) {
	if _, err := s.repo.GetCourse(ctx, customerID, courseID); err != nil {
		return httpx.Paginated[ResourceView]{}, err
	}
...
func (s *Service) UploadResource(ctx context.Context, customerID, courseID, userID int, lessonID *int, name, contentType string, r io.Reader, size int64) (*ResourceView, error) {
	if s.storage == nil {
...
func (s *Service) UploadThumbnail(ctx context.Context, customerID, courseID int, name, contentType string, r io.Reader, size int64) (*CourseView, error) {
	if s.storage == nil {
		return nil, ErrStorageUnavailable
	}
...
func (s *Service) courseView(c *Course) CourseView {
	v := CourseView{
...
func (s *Service) resourceView(r *Resource) ResourceView {
	url := ""
	if s.storage != nil {
...
func moduleViews(mods []CourseModule) []ModuleView {
...
func moduleView(m CourseModule) ModuleView {
	lessons := make([]LessonView, 0, len(m.Lessons))
	for _, l := range m.Lessons {
		lessons = append(lessons, lessonView(l))
	}
	return ModuleView{ID: m.ID, Title: m.Title, Position: m.Position, Lessons: lessons}
}

func lessonView(l Lesson) LessonView {
	return LessonView{ID: l.ID, Title: l.Title, ContentMD: l.ContentMD, Position: l.Position}
}
